...
inline T kernelFqinv(T q) {
	...
	w1 = T(-243.0 / 20.0);
	w1 *= q;
	w1 = fmaf(w1, q, T(27.0 / 4.0));
	w2 = T(243.0 / 40.0);
	w2 = fmaf(w2, q, T(-243.0 / 16.0));
	w2 = fmaf(w2, q, T(81.0 / 8.0));
	w2 -= T(1.0 / 80.0) / (sqr(q) * q + T(1e-30));
	sw1 = (q < T(1.0 / 3.0));
	sw2 = T(1) - sw1;
	res = w1 * sw1 + w2 * sw2;
	sw1 = q0 < T(1);
...
inline T kernelPot(T q) {
	...
	w1 = T(243.0 / 80.0);
	w1 *= q;
	w1 = fmaf(w1, q, T(-27.0 / 8.0));
	w1 *= q;
	w1 = fmaf(w1, q, T(39.0 / 16.0));
	w2 = T(-243.0 / 160.0);
	w2 = fmaf(w2, q, T(81.0 / 16.0));
	w2 = fmaf(w2, q, T(-81.0 / 16.0));
	w2 *= q;
	w2 = fmaf(w2, q, T(81.0 / 32.0));
	w2 -= T(1.0 / 80.0) / (q + T(1e-30));
	sw1 = (q < T(1.0 / 3.0));
	sw2 = T(1) - sw1;
	res = w1 * sw1 + w2 * sw2;
	sw1 = q0 < T(1);
	sw2 = T(1) - sw1;
	res = (sw1 * res + sw2 / (q0 + T(1e-30))) * (q0 > T(0));
	return res;
}
