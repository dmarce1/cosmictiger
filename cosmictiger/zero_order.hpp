...
struct cosmic_params {
	double omega_b, omega_c, omega_gam, omega_nu, omega_lam;
	double Neff, Y, Theta, hubble;
};

inline double hubble_function(double a, double littleh, double omega_m, double omega_r, double omega_lam) {
	return littleh * cosmic_constants::H0 * sqrt(omega_r / (a * a * a * a) + omega_m / (a * a * a) + omega_lam + (1.0 - omega_lam - omega_r - omega_m) / (a * a));
}
...
class cosmos {
	...
	double H;
public:
	cosmos(double omega_c, double omega_b, double omega_gam, double omega_nu, double olam, double h_, double a_) {
		H = constants::H0 * h_;
		omega_m = omega_c + omega_b;
		omega_r = omega_gam + omega_nu;
		omega_lam = olam;
		omega_k = 1.0 - omega_lam - omega_r - omega_m;
		a = a_;
		t = tau = 0.0;
	}
	double advance(double dtau0) {
		...
		while (tau < tau1) {
			double a0 = a;
			double t0 = t;
			double dtau = std::min(std::abs(1.0e-2 / a / (H * sqrt(omega_m / (a * a * a) + omega_r / (a * a * a * a) + omega_lam + omega_k / (a * a)))) * 0.01,
					tau1 - tau);
			for (int rk = 0; rk < 3; rk++) {
				double da = (H * sqrt(omega_m / (a * a * a) + omega_r / (a * a * a * a) + omega_lam + omega_k / (a * a))) * a * a * dtau;
				double dt = a * dtau;
