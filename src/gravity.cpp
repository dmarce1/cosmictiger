...
size_t cpu_gravity_pp(gravity_cc_type gtype, force_vectors& f, int min_rung, tree_id self, const vector<tree_id>& list, float hfloat) {
	...
	if (list.size()) {
		...
		for (int li = 0; li < list.size(); li += chunk_size) {
			...
			srcz.resize(nsource);
			masses.resize(nsource);
			vsofts.resize(nsource);
			type.resize(nsource);
			int count = 0;
			for (int i = 0; i < maxi; i++) {
				particles_global_read_pos(tree_ptrs[i]->global_part_range(), srcx.data(), srcy.data(), srcz.data(), type.data(), vsoft ? vsofts.data() : nullptr,
						count);
				count += tree_ptrs[i]->nparts();
			}
			if (do_sph) {
