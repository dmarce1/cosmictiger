...
#ifdef ENTROPY
				const float eint_i = A_i * powf(rho_i, gamma0 - 1.0f);		// 11
#endif
				float gradx = 0.0f;
				float grady = 0.0f;
				float gradz = 0.0f;
...
#ifdef ENTROPY
						const float eint_j = A_j * powf(rho_j, gamma0 - 1.0f); // 11
#endif
...
#ifdef ENTROPY
					const float eint = code_to_energy * A_i * powf(rho0, gamma0 - 1.0) * invgm1; // 13
#else
							const float eint = eint_i;
#endif
