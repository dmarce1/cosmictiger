		...
		while (i < partsz) {
			...
			for (int k = tid; k < nactive; k += WARP_SIZE) {
				...
				for (int j = 0; j < part_index; j++) {
					...
					const float mass = sph ? (src_fpot[j] != 0.f ? sph_mass : dm_mass) : 1.f;
