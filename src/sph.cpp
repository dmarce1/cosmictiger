...
sph_run_return sph_run_workspace::to_gpu() {
						...
						[&index,proc,nthreads,this,&part_index, chem, stars, conduction, diffusion]() {
							...
							while( this_index < host_trees.size()) {
								...
								switch(params.run_type) {
									...
									case SPH_RUN_HYDRO:
									sph_particles_global_read_sph(node.global_part_range(), params.a, host_eint.data(), host_vx.data(), host_vy.data(), host_vz.data(), (chem||params.conduction) ? host_gamma.data() : nullptr,host_alpha.data(), nullptr, diffusion ? host_chem.data() : nullptr, offset);
									break;
									case SPH_RUN_AUX: {
								...
								switch(params.run_type) {
									case SPH_RUN_HYDRO:
									sph_particles_global_read_aux(node.global_part_range(), host_fpre.data(), host_fpot.data(), nullptr, host_balsara.data(), host_shearv.data(), nullptr, offset);
									break;
									case SPH_RUN_AUX:
									if( params.phase == 1 ) {
										sph_particles_global_read_aux(node.global_part_range(), nullptr, nullptr, host_divv.data(), nullptr, nullptr, nullptr, offset);
									}
									break;
	...
	if (params.run_type == SPH_RUN_AUX || params.run_type == SPH_RUN_XSPH) {
		...
		CUDA_CHECK(cudaMemcpyAsync(cuda_data.fpot, host_fpot.data(), sizeof(float) * host_fpot.size(), cudaMemcpyHostToDevice, stream));
