						...
						[&index,proc,nthreads,this,&part_index, chem, stars, conduction]() {
							...
							while( this_index < host_trees.size()) {
								...
								switch(params.run_type) {
									...
									sph_particles_global_read_sph(node.global_part_range(), params.a, host_eint.data(), host_vx.data(), host_vy.data(), host_vz.data(), params.conduction ? host_gamma.data() : nullptr, nullptr, nullptr, chem ? host_chem.data() : nullptr, offset);
									break;
									case SPH_RUN_HYDRO:
								...
								switch(params.run_type) {
									case SPH_RUN_HYDRO:
									sph_particles_global_read_aux(node.global_part_range(), host_fpre.data(), nullptr, host_balsara.data(), nullptr, nullptr, offset);
									break;
									case SPH_RUN_AUX:
									if( params.phase == 1 ) {
					...
					if( self->nactive || has_active_neighbors(self)) {
						for( part_int j = self->part_range.first; j < self->part_range.second; j++) {
							...
							if( rung >= minrung || sa) {
								...
								e1 += de;
								this_error = std::max(this_error,fabs(de)/std::max(e0,e1));
								if( chem ) {
