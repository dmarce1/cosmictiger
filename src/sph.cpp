...
sph_run_return sph_run_workspace::to_gpu() {
	...
	} else if (params.run_type == SPH_RUN_PREHYDRO) {
		host_h.resize(parts_size);
				...
				hpx::async(
						[&index,proc,nthreads,this,&part_index, chem, stars, conduction, diffusion]() {
							...
							while( this_index < host_trees.size()) {
								...
								if( params.run_type == SPH_RUN_PREHYDRO) {
									sph_particles_global_read_eint_and_smoothlen(node.global_part_range(), nullptr, host_h.data(),offset);
									...
									sph_particles_global_read_eint_and_smoothlen(node.global_part_range(), host_eint.data(), host_h.data(), offset);
									sph_particles_global_read_vels(node.global_part_range(), host_vx.data(), host_vy.data(), host_vz.data(), offset);
									sph_particles_global_read_aux(node.global_part_range(),host_alpha.data(), host_fpre1.data(), diffusion ? host_shearv.data() : nullptr, chem ? host_chem.data() :nullptr, offset);
								} else if( params.run_type == SPH_RUN_AUX) {
									sph_particles_global_read_vels(node.global_part_range(), host_vx.data(), host_vy.data(), host_vz.data(), offset);
									...
									sph_particles_global_read_rungs(node.global_part_range(), host_rungs.data(), offset);
									sph_particles_global_read_eint_and_smoothlen(node.global_part_range(), host_eint.data(),host_h.data(), offset);
									sph_particles_global_read_aux(node.global_part_range(), nullptr,  host_fpre1.data(),nullptr, nullptr, offset);
								}
		...
		futs.push_back(hpx::async([proc, nthread,minrung] {
			...
			for( part_int i = b; i < e; i++) {
				if( !sph_particles_converged(i) && sph_particles_rung(i) >= minrung || sph_particles_semiactive(i)) {
					...
					const float dA = sph_particles_deint(i);
