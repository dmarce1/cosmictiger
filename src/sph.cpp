...
sph_run_return sph_run_workspace::to_gpu() {
				...
				hpx::async(
						[&index,proc,nthreads,this,&part_index, chem, stars]() {
							...
							while( this_index < host_trees.size()) {
								...
								switch(params.run_type) {
									case SPH_RUN_DEPOSIT:
									sph_particles_global_read_sph(node.global_part_range(), nullptr, host_vx.data(), host_vy.data(), host_vz.data(), host_gamma.data(), nullptr, nullptr, offset);
									break;
									case SPH_RUN_HYDRO:
								...
								switch(params.run_type) {
									case SPH_RUN_DEPOSIT:
									if( stars ) {
										sph_particles_global_read_sns(node.global_part_range(), host_sn.data(), offset);
									}
									break;
								}
								switch(params.run_type) {
	...
	case SPH_RUN_COURANT:
		if (stars) {
			...
			CUDA_CHECK(cudaMemcpyAsync(cuda_data.Y, host_Y.data(), sizeof(float) * host_Y.size(), cudaMemcpyHostToDevice, stream));
			CUDA_CHECK(cudaMemcpyAsync(cuda_data.Z, host_Z.data(), sizeof(float) * host_Z.size(), cudaMemcpyHostToDevice, stream));
		}
		break;
	}
	switch (params.run_type) {
	...
	case SPH_RUN_DEPOSIT:
		if (stars) {
			CUDA_CHECK(cudaFree(cuda_data.sn));
		}
		break;
	}
	switch (params.run_type) {
...
void sph_deposit_sn(float a) {
	...
	for (int proc = 0; proc < nthreads; proc++) {
		futs.push_back(hpx::async([proc,nthreads,a,&count1,&count2]() {
			vector<float> sn;
			vector<fixed32> x;
			vector<fixed32> y;
			vector<fixed32> z;
