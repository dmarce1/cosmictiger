...
vector<float, pinned_allocator<float>> kick_workspace::host_mass;
vector<float, pinned_allocator<float>> kick_workspace::host_hsoft;
...
void kick_workspace::to_gpu() {
	...
	const static bool vsoft = sph && get_options().vsoft;
#ifdef USE_CUDA
	timer tm;
	lock1.wait();
	cuda_set_device();
...
//	PRINT("%i tree ids\n", tree_ids_vector.size());
		...
		futs.push_back(hpx::async(HPX_PRIORITY_HI, [&next_index,&tree_ids_vector,&tree_map,proc,nthreads,&tree_bases]() {
							for (int i = proc; i < tree_ids_vector.size(); i+=nthreads) {
								if( tree_bases.find(tree_ids_vector[i]) != tree_bases.end()) {
									...
									particles_global_read_pos(ptr->global_part_range(), host_x.data(), host_y.data(), host_z.data(), vsoft ? host_hsoft.data() : nullptr, sph ? host_mass.data() : nullptr, part_index);
	...
	if( sph ) {
		CUDA_CHECK(cudaMalloc(&dev_mass, sizeof(char) * part_count));
		if( vsoft ) {
			CUDA_CHECK(cudaMalloc(&dev_hsoft, sizeof(float) * part_count));
		}
	}
	if( sph ) {
		CUDA_CHECK(cudaMemcpyAsync(dev_mass, host_mass.data(), sizeof(float) * part_count, cudaMemcpyHostToDevice, stream));
		if( vsoft ) {
			CUDA_CHECK(cudaMemcpyAsync(dev_hsoft, host_hsoft.data(), sizeof(float) * part_count, cudaMemcpyHostToDevice, stream));
		}
	}
...
//	PRINT("parts size = %li\n", sizeof(fixed32) * part_count * NDIM);
	const auto kick_returns = cuda_execute_kicks(params, dev_x, dev_y, dev_z, dev_hsoft, dev_mass, dev_trees, std::move(workitems), stream, part_count, tree_nodes.size(), [&]() {lock2.wait();}, [&]() {lock1.signal();});
	cuda_end_stream(stream);
//	PRINT("To GPU Done %i\n", hpx_rank());
	if( sph ) {
		CUDA_CHECK(cudaFree(dev_mass));
		if( vsoft ) {
