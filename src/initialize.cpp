...
static float zeldovich_end(int dim, bool init_parts, float D1, float prefac1) {
	...
	if (init_parts) {
		...
		for (I[0] = box.begin[0]; I[0] != box.end[0]; I[0]++) {
			local_futs.push_back(hpx::async([box,Ninv,sph](array<int64_t,NDIM> I) {
				...
				const double omega = omega_b + omega_m;
				const double dx = 1.0 / get_options().parts_dim;
				const double shift_gas = -0.5 * omega_m / omega * dx;
				const double shift_dm = +0.5 * omega_b / omega * dx;
				for (I[1] = box.begin[1]; I[1] != box.end[1]; I[1]++) {
					for (I[2] = box.begin[2]; I[2] != box.end[2]; I[2]++) {
						const int64_t index = box.index(I);
	...
	for (I[0] = box.begin[0]; I[0] != box.end[0]; I[0]++) {
		futs.push_back(hpx::async([box, D1, prefac1, dim, &Y, box_size_inv, N,sph](array<int64_t,NDIM> I) {
