...
//					PRINT( "%e %i\n", eps, make_star);
				if( make_star ) {
					if( false && gsl_rng_uniform(rnd_gens[proc]) < BETA_SN) {
						...
						float Htot = H.to_double() + Hp.to_double() + Hn.to_double() + H2.to_double();
						float Hetot = He.to_double() + Hep.to_double() + Hepp.to_double();
						Hetot += HE_SN;
						Z = Z.to_double() + Z_SN;
						float norm = Hetot+ Htot+ Z.to_double();
						Hetot /= norm;
						Htot /= norm;
						Z = Z.to_double() / norm;
						H = 1e-30;
						...
						star.Z = sph_particles_Z(i).to_double();
