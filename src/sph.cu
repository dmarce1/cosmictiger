		...
		for (int i = self.part_range.first; i < self.part_range.second; i++) {
			...
			if (use) {
				...
				for (int j = tid; j < ws.rec1.size(); j += block_size) {
					...
					de_dt += 0.5f * ainv * dvisc_ij * (vx_ij * dWdr_x_ij + vy_ij * dWdr_y_ij + vz_ij * dWdr_z_ij) * m;
					ax += dvx_dt;
					ay += dvy_dt;
					az += dvz_dt;
					one += m * rhoinv_i * W_i;
					...
					if (params.phase == 1 || params.damping > 0.f) {
						vsig_ij = (c_ij - params.beta * vdotx_ij * rinv);
				...
				for (int j = tid; j < ws.rec1.size(); j += block_size) {
					...
					if (r2 < fmaxf(h2_j, h2_i)) {
						...
						if (r2 > 0.f) {
							...
							const float difco_ij = SPH_DIFFUSION_C * sqr(h_ij) * 0.5f * (shearv_i + shearv_j);
