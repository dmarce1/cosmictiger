...
//				float one = 0.0f;
				...
				for (int j = tid; j < ws.rec1.size(); j += block_size) {
					...
					const float dvisc_ij = -0.5f * mu_ij * (c_ij - params.beta * mu_ij);
					...
					const float dv_i = dvisc_ij * alpha_i * rhoinv_i;
					const float dv_j = dvisc_ij * alpha_j * rhoinv_j;
