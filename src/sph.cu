		...
		for (int i = self.part_range.first; i < self.part_range.second; i++) {
			...
			if (use) {
				...
				for (int j = tid; j < ws.rec1.size(); j += block_size) {
					...
					const float fvisc = -viscco_ij * w_ij * rinv / rho_ij;
					...
					const float dp_i = p_i * sqr(rhoinv_i) + 0.5f * fvisc;
					const float dp_j = p_j * sqr(rhoinv_j) + 0.5f * fvisc;
...
//							PRINT( "%e %e %e %e %e %e %e\n", vsig, c_ij - w_ij, c_i, c_j, gamma_j,  p_j,  rhoinv_j);
				...
				if (tid == 0) {
					...
					if (params.phase == 0) {
						...
						if (params.tau == 0.f) {
							data.divv_dt_snk[snki] = 0.f;
						} else {
							data.divv_dt_snk[snki] = (div_v - div_v0) / (params.tau - data.taux_snk[snki]);
						}
					...
					if (params.phase == 2) {
						...
						if (active) {
							const float ddivv_dt = data.divv_dt_snk[snki];
