...
struct smoothlen_workspace {
	...
	device_vector<float> vx;
	device_vector<float> vy;
	device_vector<float> vz;
};
		...
		for (int i = self.part_range.first; i < self.part_range.second; i++) {
			...
			if (use) {
				...
				if (!box_xceeded) {
					...
					float dvz_dz = 0.f;
					for (int j = tid; j < ws.x.size(); j += block_size) {
						const fixed32 x_j = ws.x[j];
						const fixed32 y_j = ws.y[j];
						const fixed32 z_j = ws.z[j];
					...
					for (int j = tid; j < ws.x.size(); j += block_size) {
						...
						const float h4inv_i = sqr(sqr(hinv_i));
						if (q < 1.f) {                               // 1
							const float vx_j = ws.vx[j];
							const float vy_j = ws.vy[j];
							const float vz_j = ws.vz[j];
		...
		for (int i = self.part_range.first; i < self.part_range.second; i++) {
			...
			if (use) {
				...
				if (tid == 0) {
					...
					if (params.diffusion) {
						dtinv_hydro1 = fmaxf(dtinv_hydro1, params.a * D * params.cfl * 1.1f);
					}
	...
	while (index < data.nselfs) {
		...
		for (int i = self.part_range.first; i < self.part_range.second; i++) {
			...
			if (use) {
				...
				shared_reduce_add<float, AUX_BLOCK_SIZE>(dx_dx);
				shared_reduce_add<float, AUX_BLOCK_SIZE>(dx_dy);
				shared_reduce_add<float, AUX_BLOCK_SIZE>(dx_dz);
				shared_reduce_add<float, AUX_BLOCK_SIZE>(dy_dy);
				shared_reduce_add<float, AUX_BLOCK_SIZE>(dy_dz);
				shared_reduce_add<float, AUX_BLOCK_SIZE>(dz_dz);
				dy_dx = dx_dy;
				dz_dx = dx_dz;
				dz_dy = dy_dz;
