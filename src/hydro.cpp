		...
		for (int iy = 0; iy < ndim; iy++) {
			for (int iz = 0; iz < ndim; iz++) {
				...
				if (r < 0.05) {
					...
					double v0 = sqrt(3.0 * f * p1 / rho0) * rand_normal();
					double nx = 2.0 * rand1() - 1.0;
					double ny = 2.0 * rand1() - 1.0;
					double nz = 2.0 * rand1() - 1.0;
					const double norm = sqrt(sqr(nx, ny, nz));
					nx /= norm;
					ny /= norm;
					nz /= norm;
					vx = v0 * nx;
