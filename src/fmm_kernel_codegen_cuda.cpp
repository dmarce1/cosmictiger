...
int compute_dx(int P, const char* name = "X", bool trless = false) {
	array<int, NDIM> n;
	tprint("\".reg .f32 %%xx<%i>;\"\n", (P + 1) * P * (P + 2) / 6);
	tprint("\"mov.f32 %%xx0, 0%llx;\"\n", float2hex(1));
	for (int dim = 0; dim < NDIM; dim++) {
		tprint("\"mov.f32 %%xx%i, %%XX%i;\"\n", 1 + dim, dim);
	}
	int flops = 0;
				...
				for (m[0] = 0; m[0] <= n[0] / 2; m[0]++) {
					for (m[1] = 0; m[1] <= n[1] / 2; m[1]++) {
						for (m[2] = 0; m[2] <= n[2] / 2; m[2]++) {
							...
							if (first) {
								if (close21(factor)) {
									ASPRINTF(&str, "\"mov.f32 %%%s%i, %%%s%i;\"\n", oname.c_str(), trless_index(n[0], n[1], n[2], P), iname.c_str(),
											sym_index(p[0], p[1], p[2]));
								} else if (close21(-factor)) {
									ASPRINTF(&str, "\"neg.ftz.f32 %%%s%i, %%%s%i;\"\n", oname.c_str(), trless_index(n[0], n[1], n[2], P), iname.c_str(),
											sym_index(p[0], p[1], p[2]));
									flops++;
								} else {
									ASPRINTF(&str, "\"mul.ftz.f32 %%%s%i, 0%lx, %%%s%i;\"\n", oname.c_str(), trless_index(n[0], n[1], n[2], P), float2hex(factor),
											iname.c_str(), sym_index(p[0], p[1], p[2]));
									flops++;
								}
								asn.push_back(str);
								free(str);
								...
								if (close21(factor)) {
									ASPRINTF(&str, "\"add.ftz.f32 %%%s%i, %%%s%i, %%%s%i;\"\n", oname.c_str(), trless_index(n[0], n[1], n[2], P), oname.c_str(),
											trless_index(n[0], n[1], n[2], P), iname.c_str(), sym_index(p[0], p[1], p[2]));
									flops += 1;
								} else if (close21(-factor)) {
									ASPRINTF(&str, "\"sub.ftz.f32 %%%s%i, %%%s%i, %%%s%i;\"\n", oname.c_str(), trless_index(n[0], n[1], n[2], P), oname.c_str(),
											trless_index(n[0], n[1], n[2], P), iname.c_str(), sym_index(p[0], p[1], p[2]));
									flops += 1;
								} else {
									ASPRINTF(&str, "\"fma.rn.ftz.f32 %%%s%i, 0%lx, %%%s%i, %%%s%i;\"\n", oname.c_str(), trless_index(n[0], n[1], n[2], P),
											float2hex(factor), iname.c_str(), sym_index(p[0], p[1], p[2]), oname.c_str(), trless_index(n[0], n[1], n[2], P));
									flops += 1;
								}
								op.push_back(str);
								free(str);
	...
	for (n[0] = 0; n[0] < Q; n[0]++) {
		for (n[1] = 0; n[1] < Q - n[0]; n[1]++) {
			for (n[2] = 0; n[2] < Q - n[0] - n[1]; n[2]++) {
				tprint("\".reg .f32 %%%s%i%i%i;\"\n", name.c_str(), n[0], n[1], n[2]);
			}
		}
	}
	for (n[0] = 0; n[0] < Q; n[0]++) {
		for (n[1] = 0; n[1] < Q - n[0]; n[1]++) {
			for (n[2] = 0; n[2] < Q - n[0] - n[1]; n[2]++) {
				if (!(n[2] >= 2 && !(n[0] == 0 && n[1] == 0 && n[2] == 2))) {
					tprint("\"ld.f32 %%%s%i%i%i, [%%%sptr+%i];\" \n", name.c_str(), n[0], n[1], n[2], name.c_str(),
							sizeof(float) * trless_index(n[0], n[1], n[2], Q));
				}
			}
		}
	}
			...
			for (n[2] = 0; n[2] < Q - n[0] - n[1]; n[2]++) {
				if ((n[2] >= 2 && !(n[0] == 0 && n[1] == 0 && n[2] == 2))) {
					...
					for (int i = 0; i < counts.N; i++) {
						if (counts[i] == 1) {
							if (first) {
								tprint("\"ld.f32 %%%s%i%i%i,[%%%sptr+%i];\"\n", name.c_str(), n[0], n[1], n[2], name.c_str(), sizeof(float) * i);
							} else {
								tprint("\"ld.f32 %%tmp0, [%%%sptr+%i];\" \n", name.c_str(), sizeof(float) * i);
								tprint("\"add.ftz.f32 %%%s%i%i%i, %%%s%i%i%i, %%tmp0;\"\n", name.c_str(), n[0], n[1], n[2], name.c_str(), n[0], n[1], n[2]);
							}
							first = false;
							sign = signs[i];
						} else if (counts[i]) {
							if (!first) {
								tprint("\"ld.f32 %%tmp0, [%%%sptr+%i];\" \n", name.c_str(), sizeof(float) * i);
								tprint("\"fma.rn.ftz.f32 %%%s%i%i%i, %%tmp0, 0%llx, %%%s%i%i%i;\"\n", name.c_str(), n[0], n[1], n[2], float2hex(counts[i]),
										name.c_str(), n[0], n[1], n[2]);
							} else {
								tprint("\"ld.f32 %%%s%i%i%i, [%%%sptr+%i];\" \n", name.c_str(), n[0], n[1], n[2], name.c_str(), sizeof(float) * i);
								tprint("\"mul.ftz.f32 %%%s%i%i%i, 0%llx, %%tmp0;\"\n", name.c_str(), n[0], n[1], n[2], float2hex(counts[i]));
							}
							first = false;
							sign = signs[i];
						}
					...
					if (sign < 0) {
						tprint("\"neg.ftz.f32 %%%s%i%i%i, %%%s%i%i%i;\"\n", name.c_str(), n[0], n[1], n[2], name.c_str(), n[0], n[1], n[2]);
					}
					std::string cmd;
...
template<int Q>
int const_reference_trless2(std::string name, std::string name2) {
	array<int, NDIM> n;
	int flops = 0;
	for (n[0] = 0; n[0] < Q; n[0]++) {
		for (n[1] = 0; n[1] < Q - n[0]; n[1]++) {
	...
	for (n[0] = 0; n[0] < Q; n[0]++) {
		for (n[1] = 0; n[1] < Q - n[0]; n[1]++) {
			for (n[2] = 0; n[2] < Q - n[0] - n[1]; n[2]++) {
				if (!(n[2] >= 2 && !(n[0] == 0 && n[1] == 0 && n[2] == 2))) {
					tprint("\"mov.f32 %%%s%i%i%i, %%%s%i;\" \n", name.c_str(), n[0], n[1], n[2], name2.c_str(), trless_index(n[0], n[1], n[2], Q));
				}
			}
		}
	}
			...
			for (n[2] = 0; n[2] < Q - n[0] - n[1]; n[2]++) {
				if ((n[2] >= 2 && !(n[0] == 0 && n[1] == 0 && n[2] == 2))) {
					...
					for (int i = 0; i < counts.N; i++) {
						if (counts[i] == 1) {
							if (first) {
								tprint("\"mov.f32 %%%s%i%i%i,%%%s%i;\"\n", name.c_str(), n[0], n[1], n[2], name2.c_str(), i);
							} else {
								tprint("\"add.ftz.f32 %%%s%i%i%i, %%%s%i%i%i, %%%s%i;\"\n", name.c_str(), n[0], n[1], n[2], name.c_str(), n[0], n[1], n[2], name2.c_str(), i);
							}
							first = false;
							sign = signs[i];
						} else if (counts[i]) {
							if (!first) {
								tprint("\"fma.rn.ftz.f32 %%%s%i%i%i, %%%s%i, 0%llx, %%%s%i%i%i;\"\n", name.c_str(), n[0], n[1], n[2], name2.c_str(), i, float2hex(counts[i]),
										name.c_str(), n[0], n[1], n[2]);
							} else {
								tprint("\"mov.f32 %%%s%i%i%i, %%%s%i;\" \n", name.c_str(), n[0], n[1], n[2], name2.c_str(), i);
								tprint("\"mul.ftz.f32 %%%s%i%i%i, 0%llx, %%tmp0;\"\n", name.c_str(), n[0], n[1], n[2], float2hex(counts[i]));
...
void m2l() {
	...
	tprint("__device__ inline void M2L_cuda(const int* list, int list_size, const tree_node* nodes, unsigned  do_phi, const fixed32* X, float* L) {\n", P * P + 1, (P - 1) * (P - 1) + 1,
	P * P + 1);
	indent();
	tprint("asm volatile(\"{\"\n", 2);
	...
	tprint("\".reg .f32 %%expansion<%i>;\"\n", P * P + 1);
	tprint("\".reg .u32 %index<2>;\"\n");
	tprint("\".reg .u32 %end;\"\n");
	tprint("\".reg .u32 %snk<3>;\"\n");
	tprint("\".reg .f32 %%XX<%i>;\"\n", 3);
	tprint("\".reg .u32 %src;\"\n");
	tprint("\".reg .s32 %idx;\"\n");
	tprint("\".reg .u64 %ptr<1>;\"\n");
	tprint("\".reg .pred %%pred<%i>;\"\n", 1);
	tprint("\".reg .u64 %multiptr;\"\n");

	for (int dim = 0; dim < NDIM; dim++) {
		tprint("\"ld.u32 %%snk%i, [%%4+%i];\"\n", dim, sizeof(fixed32) * dim);
	}
	for (int i = 0; i < P * P + 1; i++) {
		tprint("\"mov.f32 %%expansion%i, 0%llx;\"\n", i, float2hex(0.0));
	}

	tprint("\"mov.u32 %index0, %tid.x;\"\n");
	tprint("\"mov.u32 %end, %1;\"\n");
	printf("\"M2L_LOOP_BEGIN:\"\n");
	tprint("\"setp.ge.u32 %pred0, %index0, %end;\"\n");
	tprint("\"@%pred0 bra M2L_LOOP_END;\"\n");
	tprint("\"cvt.u64.u32 %ptr0, %index0;\"\n");
	tprint("\"mul.lo.u64 %ptr0, %ptr0, 4;\"\n");
	tprint("\"add.u64 %ptr0, %ptr0, %0;\"\n");
	tprint("\"ld.u32 %index1, [%ptr0];\"\n");
	tprint("\"cvt.u64.u32 %ptr0, %index1;\"\n");
	tprint("\"mul.lo.u64 %%ptr0, %%ptr0, %i;\"\n", sizeof(tree_node));
	tprint("\"add.u64 %ptr0, %ptr0, %2;\"\n");
	tprint("\"ld.u64 %ptr0, [%ptr0];\"\n");
	for (int dim = 0; dim < NDIM; dim++) {
		tprint("\"ld.u32 %%src, [%%ptr0+%i];\"\n", sizeof(fixed32) * dim);
		tprint("\"sub.u32 %%idx, %%snk%i, %%src;\"\n", dim);
		tprint("\"cvt.rn.f32.s32 %%XX%i, %%idx;\"\n", dim);
		tprint("\"mul.ftz.f32 %%XX%i, %%XX%i, 0%llx;\"\n", dim, dim, float2hex(fixed2float));
	}
	tprint("\"add.u64 %%ptr0, %%ptr0, %i;\"\n", sizeof(fixed32) * NDIM);
	tprint("\"mov.u64 %multiptr, %ptr0;\"\n");

	tprint("\".reg .f32 %%tmp<%i>;\"\n", 2);
	tprint("\".reg .f32 %%rinv<%i>;\"\n", P + 1);
	tprint("\".reg .u32 %%Uu<%i>;\"\n", 1);
	tprint("\"prefetchu.L1 [%multiptr];\"\n");
	tprint("\"prefetchu.L1 [%multiptr+128];\"\n");
	tprint("\"mul.ftz.f32 %rinv1, %XX0, %XX0;\"\n");
	tprint("\"fma.rn.ftz.f32 %rinv1, %XX1, %XX1, %rinv1;\"\n");
	tprint("\"fma.rn.ftz.f32 %rinv1, %XX2, %XX2, %rinv1;\"\n");
	tprint("\"setp.ne.f32 %%pred0, %%rinv1, 0%llx;\"\n", float2hex(0));
	tprint("\"@%pred0 rsqrt.approx.ftz.f32 %rinv1, %rinv1;\"\n");
	tprint("\"neg.ftz.f32 %rinv1, %rinv1;\"\n");
	for (int i = 1; i < P; i++) {
		const int j = i / 2;
		const int k = i - j;
		tprint("\"mul.ftz.f32 %%rinv%i, %rinv%i, %%rinv%i;\"\n", i + 1, j + 1, k);
		tprint("\"neg.ftz.f32 %%rinv%i, %%rinv%i;\"\n", i + 1, i + 1);
	}
	for (int dim = 0; dim < NDIM; dim++) {
		tprint("\"mul.ftz.f32 %%XX%i, %%XX%i, %%rinv1;\"\n", dim, dim);
	}
	compute_dx(P, "xx", true);
	compute_detraceD<P>("xx", "greentrless");
	...
	for (k[0] = 0; k[0] < P; k[0]++) {
		for (k[1] = 0; k[1] < P - k[0]; k[1]++) {
			...
			for (k[2] = 0; k[2] < zmax; k[2]++) {
				const int k0 = k[0] + k[1] + k[2];
				tprint("\"mul.ftz.f32 %%greentrless%i, %%greentrless%i, %%rinv%i;\"\n", trless_index(k[0], k[1], k[2], P), trless_index(k[0], k[1], k[2], P), k0 + 1);
			}
		}
	}

	const_reference_trless2<P>("green", "greentrless");
	const_reference_trless<P - 1>("multi");
	array<int, NDIM> n;
	array<int, NDIM> m;
				...
				for (m[0] = 0; m[0] < q0; m[0]++) {
					for (m[1] = 0; m[1] < q0 - m[0]; m[1]++) {
						for (m[2] = 0; m[2] < q0 - m[0] - m[1]; m[2]++) {
							...
							e.n = n;
							e.m = m;
							const int index = trless_index(n[0], n[1], n[2], P);
	...
	std::vector<int> first(P * P + 1, true);
	for (int i = 0; i < 1; i++) {
		double last_coeff = 1.0;
		const auto n = entries[i][0].n;
		const int nindex = trless_index(n[0], n[1], n[2], P);
		auto& fl = phi_flops;
		tprint("\"mov.u32 	%Uu0, %3;\"\n");
		tprint("\"setp.eq.u32 	%pred0, %Uu0, 0;\"\n");
		tprint("\"@%pred0 bra M2L_BEGIN_REST;\"\n");
		//		tprint("if( do_phi ) {\n");
		//		indent();
		for (int j = 0; j < entries[i].size(); j++) {
			...
			if (!close21(last_coeff / coeff)) {
				...
				if (!close21(factor)) {
					tprint("\"mul.ftz.f32 %%expansion%i, %%expansion%i, 0%llx;\"\n", nindex, nindex, float2hex(factor));
				}
				last_coeff = coeff;
				fl++;
			}
			tprint("\"fma.rn.ftz.f32 %%expansion%i, %%multi%i%i%i, %%green%i%i%i, %%expansion%i;\"\n", nindex, m[0], m[1], m[2], n[0] + m[0], n[1] + m[1], n[2] + m[2], nindex);
			fl += 2;
		}
		if (!close21(last_coeff)) {
			fl++;
			tprint("\"mul.ftz.f32 %%expansion%i, %%expansion%i, 0%llx;\"\n", nindex, nindex, last_coeff);
		}
		if (nindex == 0) {
			//			deindent();
			//			tprint("}\n");
	...
	tprint("\"add.u32 %%index0, %%index0, %i;\"\n", WARP_SIZE);
	tprint("\"bra M2L_LOOP_BEGIN;\"\n");
	printf("\"M2L_LOOP_END:\"\n");
	for( int i = 0; i < P * P + 1; i++) {
		tprint( "\"st.f32 [%%5+%i], %%expansion%i;\"\n", i*sizeof(float), i);
	}
	deindent();
	tprint("\"}\"\n");
	tprint(": : \"l\"(list), \"r\"(list_size), \"l\"(nodes), \"r\"(do_phi), \"l\"(X) , \"l\"(L) : \"memory\");\n");
	deindent();
	tprint("} \n");
